  ...
  Mutation: {
    async createRole(_, { name }) {
      try {
        ...
        for (let i = 0; i < TYPES.length; i++) {
          let roleTypeCombo;
          TYPES[i] === ""
            ? (roleTypeCombo = newRole.name)
            : (roleTypeCombo = newRole.name + "-" + TYPES[i]);
    ...
    async deleteRole(_, { name }) {
      try {
        ...
        const resT1 = await RoleHasPermissions.deleteOne({
          role: name + "-T1",
        });
        const resT2 = await RoleHasPermissions.deleteOne({
          role: name + "-T2",
        });
        const resT3 = await RoleHasPermissions.deleteOne({
          role: name + "-T3",
        });
    ...
    async updatePagePermission(_, { key, permission }) {
      try {
        const existingPer = await Permission.findOneAndUpdate(
          { key },
          { permission },
          {
            new: true,
        ...
        // console.log(existingPer)
        return {
          ...existingPer._doc,
        };
      } catch (error) {
        throw new GraphQLError(
          `Failed to update the page-permission: ${error.message}`
        );
      }
    },

    // Update RoleHasPermissions where all the permissions to the role is assigned
    async updateRoleHasPermissions(_, { inputPermissions }) {
      try {
        const { inputRole, inputPerm } = inputPermissions;

        const existingRole = await RoleHasPermissions.findOne({
          role: inputRole,
        });
        if (!existingRole) {
        ...
        const res = await RoleHasPermissions.findOneAndUpdate(
          { role: inputRole },
  ...
  Query: {
    ...
    async getAllRoles() {
      try {
        ...
        for (let i = 0; i < roles.length; i++) {
          roleTypeCombo.push(roles[i].role);
        }
    ...
    async getAvailablePermissions() {
      try {
        ...
        if (!availPermissions) {
          throw new GraphQLError(`No entries!`);
        }

        return availPermissions;
      } catch (err) {
        throw new GraphQLError(
          `Failed to fetch the permissions: ${err.message}`
        );
      }
    },
